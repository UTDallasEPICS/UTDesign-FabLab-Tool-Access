...
def timer(user_type):
    ...
    if user_type == 'default':
        ...
        # stores data of how long user used machine
        session_usage = f'{current_user.number}, 0, "{machine_name}", CURRENT_DATE(), "{start_time}", "{stop_time}"'
        print(session_usage)
...
def send_data(data):
    ...
    server_address = ('192.168.254.38', 2222)
    buffer_size = 1024
    udp_client = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    udp_client.settimeout(10)  # testing 10 second unresponsive
    try:
